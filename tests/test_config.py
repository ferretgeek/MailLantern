...
from mail_lantern.config import AppConfig, ConfigError
...
class ConfigTests(unittest.TestCase):
    ...
    def test_explicit_token_is_preserved(self) -> None:
        token = "l" * 32
        with patch.dict(os.environ, {"LANTERN_ACCESS_TOKEN": token}, clear=True):
            config = AppConfig.from_env()
    ...
    def test_short_token_is_rejected(self) -> None:
        with (
            patch.dict(os.environ, {"LANTERN_ACCESS_TOKEN": "short"}, clear=True),
    ...
    def test_non_loopback_http_is_rejected_by_default(self) -> None:
        env = {
            ...
            "LANTERN_ACCESS_TOKEN": "x" * 32,
            "LANTERN_ALLOWED_HOSTS": "lantern.example.com",
        }
    ...
    def test_port_bounds_are_enforced(self) -> None:
        with (
            patch.dict(os.environ, {"LANTERN_ACCESS_TOKEN": "x" * 32, "LANTERN_PORT": "70000"}, clear=True),
    ...
    def test_demo_override_wins(self) -> None:
        with patch.dict(
            os.environ,
            {"LANTERN_ACCESS_TOKEN": "x" * 32, "LANTERN_DEMO": "0"},
            clear=True,
        ):
